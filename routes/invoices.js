...
router.get("/:id", async (req, res, next) => {
    try{
        ...
        if(result.rows.length === 0){
            throw new ExpressError(`The invoice ${id} does not exist`, 404);
        }
...
router.put("/:id", async (req, res, next) => {
    try{
        ...
        const result = await db.query(`UPDATE invoices SET amt=$1, paid=$2,
        paid_date=$3 WHERE id=$4 RETURNING id, comp_code, amt, paid, add_date, paid_date`,
